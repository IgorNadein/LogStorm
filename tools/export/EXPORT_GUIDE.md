...
python export_acs_events.py --password secret --end 2025-12-15T23:59:59  --end "2025-12-31T23:59:59" \

  --out data/events.ndjson \

# Принудительная перезапись  --verbose

python export_acs_events.py --password secret --end 2025-12-15T23:59:59 --force```



# Резервная копия перед перезаписью### Экспорт с изображениями

python export_acs_events.py --password secret --end 2025-12-15T23:59:59 --force --backup
...
python export_acs_events.py --password secret --end 2025-12-15T23:59:59 --deduplicate  --pic \

  --out data/events_with_pic.ndjson \

# Дополнение существующего файла (автоматическая дедупликация)  --verbose

python export_acs_events.py --password secret --end 2025-12-15T23:59:59 --append```
...
python export_acs_events.py --password secret --end 2025-12-15T23:59:59 --resume### Решение 1: Без изображений (быстрее на 20-40%)
...
| `--start` | **обязателен** | Начальная дата (YYYY-MM-DD) |

| `--end` | **обязателен** | Конечная дата (YYYY-MM-DD) |

| `--chunk-days` | 30 | Размер периода в днях |

## Использование результатов

Экспортированные файлы можно анализировать через management CLI:

```bash
python main.py analyze --logs-file data/events.ndjson
```
...
| `--out` | events_merged.ndjson | Итоговый файл |

| `--chunks-dir` | chunks | Директория для частей |
...
python export_acs_events.py --password secret --end 2025-12-31T23:59:59 --out events.ndjson
# Ctrl+C

# 2. Продолжение с места остановки
python export_acs_events.py --password secret --end 2025-12-31T23:59:59 \
